    ...
    if defined?(Conf)
      ...
      if (File.exists?(sample_content_file_name))
        ...
        content['described_rules'].values.each do |drule|
          drule = DescribedRule.new(:rule => drule['rule'],:description => drule['description'])
          WLLogger.logger.debug "Rule : #{drule.description.inspect[0..9]}...[#{drule.rule.inspect[0..19]}] successfully added!" if drule.save
        end
      else
